...
handle_request({document_formatting, Params}, State) ->
  ...
  case els_utils:project_relative(Uri) of
    {error, not_relative} ->
      {[], State};
    RelativePath ->
      {ok, TextEdit} = case els_config:get(bsp_enabled) of
                         true ->
                           format_document_bsp(Path, RelativePath, Options);
                         false ->
                           format_document_local(Path, RelativePath, Options)
                       end,
      {TextEdit, State}
  end;
...
-spec format_document_bsp(binary(), binary(), formatting_options()) ->
        [text_edit()].
...
-spec format_document_local(binary(), binary(), formatting_options()) ->
        [text_edit()].
format_document_local(Path, RelativePath,
  ...
  Fun = fun(Dir) ->
            ...
            rebar3_formatter:format(RelativePath, default_formatter, Opts),
            OutFile = filename:join(Dir, RelativePath),
            els_text_edit:diff_files(Path, OutFile)
        end,
  tempdir:mktmp(Fun).
